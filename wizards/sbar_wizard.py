...
try:
    ...
    async def translate_text(
        text: str, target_language: str, source_language: str = "en", context: str = "medical"
    ):
...
async def get_wizard_status(wizard_id: str):
    ...
    if not session:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail=f"Wizard session not found: {wizard_id}"
        )
