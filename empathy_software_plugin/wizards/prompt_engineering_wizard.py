...
class PromptEngineeringWizard(BaseWizard):
    ...
    async def analyze(self, context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        predictions = await self._predict_prompt_degradation(
            prompt_files, version_history, context
        )
    ...
    async def _predict_prompt_degradation(
        ...
        prompts_with_versions = sum(
            1 for f in prompt_files if self._has_version_marker(f)
        )
        ...
        total_prompt_size = sum(
            self._estimate_token_count(f) for f in prompt_files
        )
