...
class AIDocumentationWizard(BaseWizard):
    ...
    async def analyze(self, context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        predictions = await self._predict_ai_confusion_points(
            doc_files, code_files, context
        )
        ...
        for code_file in code_files[:10]:  # Sample first 10
            ...
            if not self._has_docstring_examples(content):
                issues.append({
                    ...
                    "suggestion": (
                        "Add Examples section to docstrings showing actual usage"
                    )
                })
    ...
    def _find_ambiguous_phrases(self, content: str) -> List[str]:
        ...
        ambiguous = ['usually', 'normally', 'typically', 'should probably', 'might want to', 'try to']
        found = []
