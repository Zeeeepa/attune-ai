...
class MultiModelWizard(BaseWizard):
    ...
    async def analyze(self, context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Current issues
        issues = await self._analyze_model_coordination(
            model_usage, model_count, routing
        )
            ...
            try:
                with open(file_path, 'r') as f:
                    ...
                    if any(kw in content for kw in [
                        'ModelRouter', 'ModelRegistry', 'AbstractModel', 'UnifiedAPI'
                    ]):
                        return True
            except:
                pass
    ...
    def _has_cost_tracking(self, routing: List[str]) -> bool:
        ...
        for file_path in routing:
            try:
                with open(file_path, 'r') as f:
                    content = f.read()
                    if 'cost' in content.lower() and ('track' in content.lower() or 'log' in content.lower()):
                        return True
            except:
                pass
        return False
    ...
    def _has_performance_monitoring(self, routing: List[str]) -> bool:
        ...
        for file_path in routing:
            try:
                with open(file_path, 'r') as f:
                    content = f.read()
                    if any(kw in content.lower() for kw in ['latency', 'metrics', 'monitoring', 'telemetry']):
                        return True
            except:
                pass
        return False
            ...
            try:
                with open(file_path, 'r') as f:
                    ...
                    if any(kw in content.lower() for kw in [
                        'classify', 'route_by', 'select_model', 'intelligent', 'adaptive'
                    ]):
                        return True
            except:
                pass
