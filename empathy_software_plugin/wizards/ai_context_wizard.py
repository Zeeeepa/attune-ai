...
class AIContextWindowWizard(BaseWizard):
    ...
    async def analyze(self, context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Current issues
        issues = await self._analyze_current_usage(
            ai_calls, context_sources, model_limits
        )
    ...
    def _estimate_context_growth_rate(
        self,
        ai_calls: List[Dict],
        full_context: Dict
    ) -> float:
    ...
    def _calculate_avg_context_size(
        ...
        if not ai_calls:
            return 0
        total = sum(
            self._estimate_context_tokens(call, context_sources)
            for call in ai_calls
        )
    ...
    def _has_context_strategy(self, ai_calls: List[Dict]) -> bool:
        ...
        return any(
            'ContextBuilder' in code or 'context_manager' in code.lower()
