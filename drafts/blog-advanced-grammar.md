...
### Nested Strategy Implementation
...
class NestedStrategy(ExecutionStrategy):
    ...
    async def execute(
        ...
        if self.workflow_ref.workflow_id:
            workflow = get_workflow(self.workflow_ref.workflow_id)
            workflow_id = self.workflow_ref.workflow_id
        else:
            workflow = self.workflow_ref.inline
            workflow_id = f"inline_{id(workflow)}"

        # Safety checks
        if not nesting.can_nest(workflow_id):
            if nesting.current_depth >= nesting.max_depth:
                raise RecursionError(f"Maximum nesting depth ({self.max_depth}) exceeded")
            raise RecursionError(f"Cycle detected: {workflow_id} already in stack")

        # Enter nested context
        child_nesting = nesting.enter(workflow_id)
        child_context = {
            **context,
            **child_nesting.to_context({})
        }

        # Execute nested workflow
        strategy = get_strategy(workflow.strategy)
        result = await strategy.execute(workflow.agents, child_context)
...
**Tags:** ai-orchestration, conditional-logic, learning-systems, multi-agent, llm, advanced-patterns
