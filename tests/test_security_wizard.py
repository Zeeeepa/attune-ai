...
''')
            ...
            high_exploitable = [a for a in assessments if a['exploitability'] in ['CRITICAL', 'HIGH']]
...
''')
            ...
            public_assessments = [a for a in assessments if 'public_api.py' in a['vulnerability']['file_path']]
            internal_assessments = [a for a in assessments if 'internal_api.py' in a['vulnerability']['file_path']]

            # Public should have higher exploit likelihood
            if public_assessments and internal_assessments:
                assert public_assessments[0]['exploit_likelihood'] > internal_assessments[0]['exploit_likelihood']
...
''')
            ...
            endpoint_config = {
                vuln_file: {'endpoint_public': True}
            }
...
''')
            ...
            assert any('parameterized' in r.lower() or 'prepared statement' in r.lower() for r in recommendations)
...
''')
            ...
            endpoint_config = {
                critical_file: {'endpoint_public': True}
            }
...
'''
        ...
        secrets = [v for v in vulnerabilities if 'hardcoded' in v['name'].lower() or 'credential' in v['name'].lower()]
...
class TestExploitAnalyzer:
    ...
    async def test_exploit_likelihood_calculation(self):
        ...
        assert 'IMMEDIATE' in assessment.mitigation_urgency or 'URGENT' in assessment.mitigation_urgency
    ...
    async def test_internal_vs_public_accessibility(self):
        ...
        # Public endpoint
        public_assessment = analyzer.assess_exploitability(
            vulnerability,
            {'endpoint_public': True}
        )
