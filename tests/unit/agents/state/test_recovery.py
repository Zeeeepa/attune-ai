...
class TestAgentRecoveryManager:
    ...
    def test_find_interrupted_returns_empty_when_none(self, tmp_path: Path) -> None:
    ...
    def test_recover_agent_returns_none_without_checkpoint(self, tmp_path: Path) -> None:
    ...
    def test_recover_agent_returns_none_for_unknown(self, tmp_path: Path) -> None:
        """Test that recovery returns None for unknown agents."""
        store = AgentStateStore(storage_dir=str(tmp_path))
        recovery = AgentRecoveryManager(store)
        assert recovery.recover_agent("nonexistent") is None

    def test_mark_abandoned_updates_running_executions(self, tmp_path: Path) -> None:
    ...
    def test_mark_abandoned_skips_completed_executions(self, tmp_path: Path) -> None:
        ...
        interrupted = [e for e in state.execution_history if e.status == "interrupted"]
