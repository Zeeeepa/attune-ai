...
Try to be as helpful as possible.
    ...
    @pytest.mark.asyncio
    async def test_analyze_detects_vague_language(self, wizard, temp_prompt_file):
        ...
        assert any(
            issue['type'] == 'vague_language'
            for issue in issues
        ), "Should detect vague language like 'try to', 'help'"

    @pytest.mark.asyncio
    async def test_analyze_detects_missing_structure(self, wizard, temp_prompt_file):
        ...
        issues = result['issues']
        assert any(
            issue['type'] == 'unclear_structure'
            for issue in issues
        ), "Should detect missing role/task/context structure"

    @pytest.mark.asyncio
    async def test_level_4_predictions(self, wizard, temp_prompt_file):
    ...
    @pytest.mark.asyncio
    async def test_returns_confidence(self, wizard, temp_prompt_file):
    ...
    @pytest.mark.asyncio
    async def test_extracts_patterns(self, wizard, temp_prompt_file):
...
class TestAIContextWindowWizard:
    ...
    async def test_detects_naive_concatenation(self, wizard):
        ...
        issues = result['issues']
        assert any(
            issue['type'] == 'naive_concatenation'
            for issue in issues
...
def analyze_code(code):
    ...
    @pytest.mark.asyncio
    async def test_detects_level_1_reactive_patterns(self, wizard, temp_ai_file):
        ...
        issues = result['issues']
        assert any(
            'level_1' in issue.get('type', '') or
            issue.get('current_level', 0) == 1
            for issue in issues
        ), "Should detect Level 1 reactive patterns"

    @pytest.mark.asyncio
    async def test_assesses_maturity_level(self, wizard, temp_ai_file):
    ...
    @pytest.mark.asyncio
    async def test_provides_growth_recommendations(self, wizard, temp_ai_file):
...
Run the thing.
    ...
    @pytest.mark.asyncio
    async def test_detects_missing_architecture(self, wizard, temp_readme):
        ...
        issues = result['issues']
        assert any(
            issue['type'] == 'missing_architecture_context'
            for issue in issues
        ), "Should detect missing architecture section"

    @pytest.mark.asyncio
    async def test_detects_ambiguous_language(self, wizard, temp_readme):
...
Try to handle errors gracefully.
    ...
    @pytest.mark.asyncio
    async def test_level_4_predicts_missing_why_context(self, wizard, temp_readme):
        ...
        assert any(
            'why' in pred.get('type', '').lower() or
            'why' in pred.get('alert', '').lower()
            for pred in predictions
...
class TestWizardIntegration:
    ...
    def test_all_wizards_are_level_4(self):
        ...
        for wizard in wizards:
            assert wizard.empathy_level == 4, \
                f"{wizard.name} should be Level 4 (Anticipatory)"
